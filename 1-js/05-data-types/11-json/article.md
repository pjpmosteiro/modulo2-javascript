...
Digamos que tenemos un objeto complejo, y nos gustaría convertirlo en una cadena, enviarlo a través de la red, o simplemente enviarlo para fines de logs.

Naturalmente, tal cadena debe incluir todas las propiedades importantes.

Podríamos implementar la conversión así:
...
... Pero en el proceso de desarrollo, se agregan nuevas propiedades, las propiedades antiguas se renombran y se eliminan. Actualizar el `toString` cada vez puede convertirse en algo pesado de hacer. Podríamos intentar ejecutar las propiedades en él, pero ¿qué pasa si el objeto es complejo y tiene objetos anidados en propiedades? Tendríamos que implementar su conversión también. Y, si enviamos el objeto a través de la red, también debemos proporcionar el código para "leer" nuestro objeto en la parte del cliente.

Afortunadamente, no hay necesidad de escribir el código para manejar todo esto. Esa tarea ya ha sido resuelta.

## JSON.stringify

[JSON](http://en.wikipedia.org/wiki/JSON) (JavaScript Object Notation) es un formato general para representar valores y objetos. Se describe como el estandar [RFC 4627](http://tools.ietf.org/html/rfc4627). Initially it was made for JavaScript, but many other languages have libraries to handle it as well. So it's easy to use JSON for data exchange when the client uses JavaScript and the server is written on Ruby/PHP/Java/Whatever.

Inicialmente fue hecho para JavaScript, pero muchos otros idiomas tienen bibliotecas para manejarlo también. Por lo tanto, es fácil utilizar JSON para el intercambio de datos cuando el cliente usa JavaScript y el servidor está escrito en Ruby/PHP/Java/Lo que sea.

JavaScript proporciona métodos:

- `JSON.stringify` para convertir objetos en JSON
- `JSON.parse` para convertir JSON de nuevo a objeto.

Por ejemplo, aquí hacemos `JSON.stringify` a un estudiante:
